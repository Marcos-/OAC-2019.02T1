...
#-------------------------------------------------------------------------
#		OrganizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o e Arquitetura de Computadores - Turma C 
#			Trabalho 1 - Assembly RISC-V
#
# Nome: JÃ´natas Gomes Barbosa da Silva	Matricula: 17/0059847
# Nome: Marcos Vinicius Pereira Marques	Matricula: 14/0071989
# Nome: Tatiana Franco Pereira		Matricula: 18/0131206

.data
#menu:
	str:	.asciz "\n. Defina o numero da funÃ§Ã£o desejada: \n"	#declaracao da string "str"
...
# definicao de macros
	...
	.macro print_string($arg)
		...
		#parÃƒÂ¢metros: a0 -> endereÃƒÂ§o da string que se quer imprimir
		#retorno: imprime uma string no console
		li	a7, 4		#a7=4 -> definiÃƒÂ§ÃƒÂ£o da chamada de sistema para imprimir strings na tela
		la	a0, $arg	#a0=endereÃƒÂ§o da string "ent1"
		ecall			
	.end_macro 
	...
	.macro entradaxy2()	#Macro de entrada dos valores de Xi, Xf, Yi e Yf
		ent_xi:	print_string(ent1)
			...
			ecall			
			mv s1, a0
			bge s1, a5, ent_xi	#Caso o valor recebido seja maior ou igual a 64, ele retorna ao inÃ­cio da leitura.
			blez s1, ent_xi		#Caso o valor seja igual ou menor que zero, ele tambÃ©m retorna ao inÃ­cio da leitura.
		...
		ent_xf:	print_string(ent1)
			...
			ecall
			mv s2, a0
			bge s2, a5, ent_xf	#Caso o valor recebido seja maior ou igual a 64, ele retorna ao inÃ­cio da leitura.
			blez s2, ent_xf		#Caso o valor seja igual ou menor que zero, ele tambÃ©m retorna ao inÃ­cio da leitura.
			
		ent_yi:	print_string(yi)
			li a7, 5		#coloca yi no registrador s3
			ecall			
			mv s3, a0
			bge s3, a5, ent_yi	#Caso o valor recebido seja maior ou igual a 64, ele retorna ao inÃ­cio da leitura.
			blez s3, ent_yi		#Caso o valor seja igual ou menor que zero, ele tambÃ©m retorna ao inÃ­cio da leitura.
			
		ent_yf:	print_string(yf)
			li a7, 5		#coloca yf no registrador s4
			ecall
			mv s4, a0
			bge s4, a5, ent_yf	#Caso o valor recebido seja maior ou igual a 64, ele retorna ao inÃ­cio da leitura.
			blez s4, ent_yf		#Caso o valor seja igual ou menor que zero, ele tambÃ©m retorna ao inÃ­cio da leitura.
	.end_macro
	...
	.macro entradargb()	#Macro de entradas dos valores RGB
		red: 	print_string(ent2)
			...
			ecall			
			mv s5, a0
			bge s5, a6, red		#Caso o valor recebido seja maior, igual a 256 ou menor que zero ele retorna ao inÃ­cio da leitura.
			blt s5, zero, red		
		...
		blue:	print_string(ent2)
			...
			ecall			
			mv s6, a0
			bge s6, a6, blue	#Caso o valor recebido seja maior, igual a 256 ou menor que zero ele retorna ao inÃ­cio da leitura.
			blt s6, zero, blue
		...
		green:	print_string(ent2)	
			...
			ecall			
			mv s7, a0
			bge s7, a6, green	#Caso o valor recebido seja maior, igual a 256 ou menor que zero ele retorna ao inÃ­cio da leitura.
			blt s7, zero, green
	.end_macro
...
get_point:
#Doc: Essa função recebe os parâmetros X e Y, guarda valores auxiliares para encontrar o endereço
#da posição no bitmap e decrementar contadores. Ao encontrar o endereço dos parâmetros separa as
#cores, movendo os bits para a direita e para a esquerda, e imprime para o usuário.
...
#	int salto_x = 4;
#	string posicao = XOYO;
#	rgb val; //Esse tipo contém três valores em vetor, que são os valores RGB.
#	
#	posicao -= salto_y * y;
#	posicao += salto_x * x;
#	
#	val = getCor(posicao); //Essa função retorna uma lista de tamanho 3, onde o primeiro item é o valor R, o segundo G e o terceiro B.
#
#	printf("R: %d\n", cor[0]);
#	printf("G: %d\n", cor[1]);
#	printf("B: %d\n", cor[2]);
...
draw_point:
#Doc: Após receber os parâmetros X, Y e os valores RGB, essa função registra os valores auxiliares nos registradores para
#encontrar o endereço no bitmap dos parâmetros, através de subtrações sucessivas, para a
#coordenada Y e adições sucessivas para a coordenada X. Após encontrar o endereço, são somados
#os valorse RGB e movimenta-se 8 bits para a esquerda para o registra da cor

#----------------------------------------------------------
#void draw_point(int x, int y, rgb val){
#	int salto_y = 256;
#	int salto_x = 4;
#	string posicao = XOYO;
#	
...
#	posicao += salto_x * x;
#	
#	setColor(posicao, val); //Essa função imprime no bitmap display uma cor. Protótipo: setColor(string posicao, rgb val);
#}
#----------------------------------------------------------
	...
	corRGB: entradargb()
		...
		#soma os valores e move 8 bits pra direita, liberando espaÃƒÂ§o para o prÃƒÂ³ximo valor
		or t1, s0, s5
		slli t1, t1, 8
		or  t1, t1, s6
		slli t1, t1, 8
		...
		#registra a cor em RGB no endereco correspondente ao ponto
		li s2, 0   #cor em rgb
		or s2, s2, t1
		sw s2, 0(t5)	
...
draw_full_rectangle:
#Doc: Os parâmetros Xi, Yi, Xf, Yf são lidos, além dos valores RGB. Checa-se se as posições
#iniciais são maiores que as finais, caso sim, é realizada a troca dos valores. Após essa
#checagem e eventual troca, o endereço da posição de acordo com Xi e Yi é localizado. Após isso,
#é lançado um loop para impressão da cor RGB lida, através de um loop que percorre pixel a pixel
#dentro dos limites dos parâmetros.

#----------------------------------------------------------
#void draw_full_rectangle(int xi, int yi, int xf, int yf, rgb val){
#	int salto_y = 256;
#	int salto_x = 4;
#	string posicao = XOYO;
#	int a; //Auxiliar
...
#----------------------------------------------------------
	...
	#recebe os dados de xi, yi, xf, yf e as cores em RGB
		...
		#se s2=xf for maior q s1=xi, nÃƒÂ£o troca e pula pra conferir os valores de y
		bgeu s2, s1, conti
		#caso contrario troca e recalcula o valor da largura a fim de evitar nÃƒÂºmeros negativos
		sub a1, s1, s2
		mv t4, s1
		mv s1, s2
		mv s2, t4
	...
	colore2:li a2, 0   #cor em rgb
		...
		bgt a1, zero, colore2
		#se jÃ¡ tiver zerado, passa pra linha de cima	
		addi a3, a3, -1
		addi t4, t4, -256
...
#Doc: Similar a função anterior, esta função recebe os dados de parâmetros do usuário, Xi, Yi, Xf, Yf
#e os valores RGB. Checa-se a posição inicial tem valores maiores que a posição final e substitui os
#registradores do valores, caso sejam. Após a chegagem, o endereço da posição é localizado através de
#incrementos e decrementos sucessivos na posição (0,0). Ao localizar a posição inicial, os pixel são
#coloridos com as cores RGB registradas.

	#DeterminaÃ§Ã£o da posiÃ§Ã£o (0,0)
	lw t5, X0Y0
		
	#Entrada de dados
	entradaxy2()
			
	# Checa se a posiÃ§Ã£o Xi Ã© maior que Xf
	bgeu s2, s1, cont	#Caso nÃ£o seja, pula
	mv t4, s1		#VariÃ¡vel auxiliar para troca de valores
	mv s1, s2
	mv s2, t4
	
	#Checa se Yi Ã© maior que Yf
	cont:	bgeu s4, s3, cont2	#Caso nÃ£o seja, pula
		mv t4, s3		#VariÃ¡vel auxiliar para troca de valores
		mv s3, s4
		mv s4, t4
	
	#Conta a quantidade de pixels entre as variÃ¡veis X e as variÃ¡veis Y
	#alÃ©m de guardar cada um dos valores em dois registradores difernetes
	cont2:	mv t4, s4		
		mv s8, s2	
		sub s8, s8, s1
		mv t3, s2
		...
		#AlocaÃ§Ã£o dos dados RGB em T1 para futura impressÃ£o
		or t1, s0, s5	
		slli t1, t1, 8
		or  t1, t1, s6
		slli t1, t1, 8
		...
		#Achando endereÃ§o da posiÃ§Ã£o (Xi, Yi)
	...
	#Loop para pintar todos os pixels de (Xi,Yi) atÃ© (Xf,Yi)
	...
	#Loop para pintar todos os pixels de (Xf,Yi) atÃ© (Xf,Yf)
	...
	#Loop para pintar todos os pixels de (Xf,Yf) atÃ© (Xi,Yf)
	...
	#Loop para pintar todos os pixels de (Xi,Yf) atÃ© (Xi,Yi)
...
convert_negative:
#Doc: A função converte para negativo da imagem começa carregando o endereço inicial para alteração
#dos pixels, aloca o tamanho e lê a imagem pixel a pixel invertendo os valores RGB. Logo após
#imprime o resultado no bitmap display.
	...
	loopneg:  
		...
		lw   t4, 0(s1)   		# lÃƒÂª pixel do display
		srli t5, t4, 16
		slli t5, t5, 16
		srli t2, t4, 8
		slli t2, t2, 24
...
convert_redtones:
#Doc: Esta função, assim como a anterior, registra o tamanho da imagem e o endereço inicial para
#alteração do bitmap display. Em loop, há a leitura de todos os pixels e a exclusão dos valores
#G(reen) e B(lue). 
...
load_image:
#Doc: carrega uma imagem em formato RAW RGB para memoria
